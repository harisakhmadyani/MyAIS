package com.sambu.myais.Activity.ui.masuk;
...
public class MasukFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        listViewMasuk = view.findViewById(R.id.listViewMasuk);
...
//    }
    ...
    private void getData(String tanggalAPI) {
        ...
        if (tanggalAPI.equals("Not Send")) {
            data = dataBaseAccess.Where("vwAbsensiNew", "Terkirim = 0 AND KodeAbsen = 'Absen Masuk' ORDER BY Nama");
        } else {
            data = dataBaseAccess.Where("vwAbsensiNew", "date(Tanggal) = date('" + tanggalAPI + "') AND KodeAbsen = 'Absen Masuk' ORDER BY Nama");
        }

        if (data.getCount() == 0) {
            listViewMasuk.setAdapter(null);
        } else {
            ...
            listViewMasuk.setAdapter(new AdapterHistory(getActivity().getApplicationContext(),ID, RegNo, NIK, Nama, KodeWilayah, Kehadiran, KodeAbsen, Tanggal, KodeSubPersil, Latitude, Longitude, Foto, Terkirim,SerialDevice,AppVersion, TanggalInd, Keterangan));
            listViewMasuk.setVisibility(View.VISIBLE);
        }
    }
}
